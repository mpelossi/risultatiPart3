...
\end{Verbatim}
    ...
    \textbf{Answer:}
    Our controller samples memcached CPU usage every 1s and dynamically changes the number of cores assigned to memcached between 1 and 3. Memcached is pinned to the low-numbered cores, while Docker batch jobs are restricted to the remaining cores using cpuset updates. The policy prioritizes SLO protection: memcached
    cores are increased immediately under high per-core CPU load, while cores are released only after sustained low load. Long jobs are started early to reduce makespan, but CPU/L1i-risky jobs may be paused during high memcached load. The controller does not use the future QPS trace; it reacts only to runtime CPU signals.
    \\
     
    \item How do you decide how many cores to dynamically assign to memcached? Why?
    
    \textbf{Answer:}
    Memcached uses a fixed thread count of T*=3 and dynamically receives
Cmem in {1,2,3}. The controller starts with Cmem=2. Every second it computes memcached CPU utilization per assigned core. If utilization exceeds $85\%$ per core, Cmem is increased immediately, up to 3 cores, to protect the 0.8ms p95 latency SLO. If utilization stays below $55\%$ per core for 20 seconds, Cmem is reduced by one, down to 1 core, to give more capacity back to batch jobs. This hysteresis avoids oscillations while still reacting quickly to high QPS.
     \\

    \item How do you decide how many cores to assign each batch job? Why?

     \textbf{Answer:}
All batch jobs run only on cores not currently used by memcached. If Cmem=1, batch jobs can use cores [1,2,3]; if Cmem=2, they use [2,3]; if Cmem=3, only core [3] is available. The controller normally gives the long critical-path job most of the available batch cores and collocates it with one companion job when possible.
...
\begin{itemize}
    ...
    \item \colored{canneal}: memory-bandwidth style job; used as a companion at low/medium memcached load, but not launched while Cmem=3.
    \item \colored{freqmine}: long critical-path job; receives most available batch cores when running, but is paused first under high memcached pressure.
    \item \colored{radix}: safest short job; can run even when memcached uses 3 cores.
    \item \colored{streamcluster}: long critical-path job; started early and receives most available batch cores.
...
Threads are set equal to the number of assigned cores, capped by the per-job maximum. This avoids oversubscription inside a Docker cpuset.

\begin{itemize}
    \item \colored{barnes}: min(assigned cores, 4)
    \item \colored{blackscholes}: min(assigned cores, 2)
    \item \colored{canneal}: min(assigned cores, 2)
    \item \colored{freqmine}: min(assigned cores, 4)
    \item \colored{radix}: min(assigned cores, 4)
    \item \colored{streamcluster}: min(assigned cores, 4)
    \item \colored{vips}: min(assigned cores, 4)
\end{itemize}
    ...
    \textbf{Answer:}
     Memcached always uses the low-numbered cores [0..Cmem-1]. Batch jobs use only the remaining high-numbered cores. With two or three batch cores available, the policy usually collocates one long job, streamcluster or freqmine, with one companion job such as canneal, radix, blackscholes, barnes, or vips. With only one batch core available, only one safe/useful job is run, preferring radix or blackscholes and avoiding canneal when Cmem=3. This keeps risky interference away from memcached during high load while still making progress on batch jobs.
     \\
...
\textbf{Answer:} The controller runs on the memcache-server VM. Memcached is controlled with taskset to update its CPU affinity. Batch jobs are launched as Docker containers with the native input dataset and fixed cpusets. While running, the controller uses Docker cpuset updates to move containers to the currently available batch
cores. If memcached per-core CPU exceeds the recovery threshold, the controller pauses the riskiest running container; it unpauses paused containers once memcached utilization falls below the recovery threshold. All start, end, pause, unpause, and core-update events are logged.
\\
...
\textbf{Answer:} The main trade-off is makespan versus memcached SLO protection. A more aggressive policy that always keeps the longest job running can reduce makespan under easy QPS intervals, but it risks collocating CPU/L1i-heavy jobs with memcached during
high-load periods. Our chosen policy is more conservative: it gives memcached cores immediately when load rises and only releases them after sustained low load. In the measured 15s-interval experiment, all seven batch jobs completed successfully in all three runs and the memcached SLO violation ratio was $0.0\%$ for each run.
\\
\end{itemize}
...
\end{Verbatim}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{ |c|c|c|c|c|} 
            ...
            \coloredcell{barnes}          & 175.12 & 5.03 \\  \hline
            \coloredcell{blackscholes}    & 109.31 & 3.58 \\  \hline
            \coloredcell{canneal}         & 277.96 & 2.75 \\  \hline
            \coloredcell{freqmine}        & 554.61 & 8.76 \\  \hline
            \coloredcell{radix}           & 42.84  & 2.84 \\  \hline
            \coloredcell{streamcluster}   &  548.66 & 8.46 \\  \hline
            \coloredcell{vips}            & 98.77  & 4.80 \\  \hline
            total time      & 1208.52 & 33.00 \\  \hline
        \end{tabular}
    \end{table}

    \textbf{Answer:}
    We ran the dynamic mcperf trace with $qps\_interval=15$ and $qps\_seed=2345$ threetimes using our dynamic controller. All seven batch jobs completed successfully in every run. The reported job runtimes exclude time spent paused. The mean makespan over the three runs was 1208.52 s with a standard deviation of 33.00 s. The memcached SLO violation ratio was 0.0\% in all three runs: run 1 = $0.0\%$, run 2 = $0.0\%$, run 3 = $0.0\%$. Therefore, the controller met the 0.8 ms p95 latency SLO while completing all batch jobs.
    \\
...
\end{Verbatim}
    ...
    \textbf{Summary:} 
    With $qps_{interval}=5$, the load changes more frequently than in Question 3, so the controller has less time to react and keeps/returns CPU capacity to memcached more aggressively. This increases the batch makespan compared to the 15 s trace, but the controller still keeps memcached below the required $3\%$ SLO violation threshold. The measured SLO violation ratio for the 5 s trace was $1.90\%$, compared with $0.41\%$ average for the original 15 s trace.
    \\
    ...
    \textbf{Answer:} The SLO violation ratio was $0.01897$, i.e. $1.90\%$.
    \\
    ...
    \textbf{Answer:} The smallest tested \texttt{$qps_{interval}$} that kept the memcached SLO violation ratio below $3\%$ was $1$ second.
    \\
